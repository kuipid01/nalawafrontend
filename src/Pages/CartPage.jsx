...
<span  onClick={ () => removeFromeCart(c.product?.id)} className=" text-[12px]  text-red-700 cursor-pointer"> DEL </span>
</div>
            </div>
           ...
           <span># {c.product.price}</span>
              ...
              <div className=' flex justify-center items-center ml-4'>
                <button onClick={() => handleQtyDec(c?.product.id ,  c.qty )} className=' p-1 px-2  border-2'>-</button>
                 <p className='mx-2'> {c.qty} </p> 
                <button  onClick={() => handleQtyInc(c?.product.id , c?.qty  )} className=' p-1 px-2   border-2'>+</button>
              </div>
              <div  className="  w-fit  flex flex-col items-center h-full justify-evenly">
                <h3 className=' text-[12px]  '>Total</h3>
                <small className='w-fit p-1 text-green-700'>#{ c.qty  * c.product.price } </small>
              </div>
